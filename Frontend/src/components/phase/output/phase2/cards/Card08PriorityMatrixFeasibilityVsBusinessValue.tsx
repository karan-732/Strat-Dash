...
export function Card08PriorityMatrixFeasibilityVsBusinessValue({ v }: { v: any }) {
  return (
    <div style={{ border: '1px solid var(--ln10)', background: 'var(--card)', borderRadius: '10px', boxShadow: '0 1px 2px var(--sh50),0 18px 34px -26px var(--sh90)', padding: '18px 19px 17px' }} data-card="phase2-08" data-card-title="PRIORITY MATRIX — FEASIBILITY VS BUSINESS VALUE">
      <div style={{ fontFamily: "'JetBrains Mono',ui-monospace,SFMono-Regular,monospace", fontSize: '11px', letterSpacing: '.16em', color: 'var(--fg3)' }}>
        08 · PRIORITY MATRIX — FEASIBILITY VS BUSINESS VALUE
      </div>
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: '20px minmax(0,1fr)', gap: '8px' }}>
        ...
        <div style={{ position: 'relative', height: '300px', overflow: 'hidden', border: '1px solid var(--ln16)', borderRadius: '9px', background: 'var(--bg)' }}>
          ...
          {(v.vis2.prio.items ?? []).map((m: any, mIndex: number) => (
            <Fragment key={mIndex}>
