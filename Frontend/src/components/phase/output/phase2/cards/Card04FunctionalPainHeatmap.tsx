...
export function Card04FunctionalPainHeatmap({ v }: { v: any }) {
  return (
    <div style={{ border: '1px solid var(--ln10)', background: 'var(--card)', borderRadius: '10px', boxShadow: '0 1px 2px var(--sh50),0 18px 34px -26px var(--sh90)', padding: '18px 19px 17px' }} data-card="phase2-04" data-card-title="FUNCTIONAL PAIN HEATMAP">
      <div style={{ fontFamily: "'JetBrains Mono',ui-monospace,SFMono-Regular,monospace", fontSize: '11px', letterSpacing: '.16em', color: 'var(--fg3)' }}>
        04 · FUNCTIONAL PAIN HEATMAP
      </div>
      <div style={{ marginTop: '13px', overflowX: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: v.vis2.heat.cols, gap: '1px', background: 'var(--ln12)', border: '1px solid var(--ln12)', minWidth: '440px' }}>
          ...
          {(v.vis2.heat.dims ?? []).map((d: any, dIndex: number) => (
            <Fragment key={dIndex}>
          ...
          {(v.vis2.heat.rows ?? []).map((r: any, rIndex: number) => (
            <Fragment key={rIndex}>
              <div style={{ display: 'grid', gridColumn: '1 / -1', gridTemplateColumns: v.vis2.heat.cols, gap: '1px', background: 'var(--ln12)' }}>
      ...
      <div style={{ marginTop: '11px', display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
        {(v.vis2.heat.legend ?? []).map((l: any, lIndex: number) => (
          <Fragment key={lIndex}>
